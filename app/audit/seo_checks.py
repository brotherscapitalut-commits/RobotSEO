...
def _finding(url, category, severity, title, description, how_to_fix=""):
    return {"url": url, "category": category, "severity": severity, "title": title, "description": description, "how_to_fix": how_to_fix}
...
def check_page_seo(url: str, html: str) -> list:
    ...
    if not viewport:
        ...
        findings.append(_finding(url, "seo", "pass", "Viewport verificado", "Meta viewport presente."))
    ...
    if "noindex" in robots_content.lower():
        ...
        findings.append(_finding(url, "seo", "pass", "Diretriz de indexação verificada", "Nenhum noindex foi detectado no meta robots."))
