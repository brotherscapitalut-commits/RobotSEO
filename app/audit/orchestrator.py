...
def _run_audit(app, audit_id: str):
    ...
    with app.app_context():
        ...
        try:
            ...
            seo_findings_count = sum(1 for f in all_findings if f.get("category") == "seo")
            ...
            audit.status = "done"
            audit.finished_at = datetime.utcnow()
            audit.summary = f"{len(pages)} páginas analisadas. {critical_count} problemas críticos encontrados. {len(recs)} oportunidades identificadas; {executable_jobs} transformadas em jobs executáveis."
            db.session.commit()

            # Do not make the audit wait for generation/publication. The executor starts now,
            # while the dashboard can immediately show the queued jobs and their final status.
            if executable_jobs:
