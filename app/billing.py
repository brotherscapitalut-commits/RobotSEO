...
def checkout():
    ...
    configured = bool(
        ...
        and current_app.config.get("STRIPE_PRICE_ID_MONTHLY")
    )
    return render_template("billing/checkout.html", price="$100/mês", stripe_configured=configured)
...
def create_checkout_session():
    ...
    price_id = current_app.config.get("STRIPE_PRICE_ID_MONTHLY")
    if not secret or not price_id:
        return jsonify({
            "ok": False,
            "error": "Stripe ainda não está configurado. Adicione STRIPE_SECRET_KEY e STRIPE_PRICE_ID_MONTHLY no ambiente."
        }), 503
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Não foi possível iniciar o checkout. Verifique a configuração do Stripe."}), 502
...
def customer_portal():
    ...
    if not secret or not current_user.stripe_customer_id:
        return jsonify({"ok": False, "error": "Portal de cobrança indisponível até a configuração do Stripe."}), 503
    stripe.api_key = secret
    try:
        ...
        return jsonify({"ok": False, "error": "Não foi possível abrir o portal de cobrança."}), 502
...
def webhook():
    ...
    if not secret or not webhook_secret:
        return "Stripe não configurado", 503
